...
def process_input(characterImgs, characterName, characterBackground):
    if characterImgs:
        if isinstance(characterImgs, dict):
            ...
            raise TypeError(
                "The charterImg parameter must be a dictionary, where the key is the name of the character, and the parameter is an image.\nExample: charterImg = {'Himeko': 'img.png'} or {'Himeko': 'img.png', 'Seele': 'img2.jpg', ...}"
            )
    ...
    if characterBackground:
        if isinstance(characterBackground, dict):
            ...
            raise TypeError(
                "The characterBackground parameter must be a dictionary, where the key is the name of the character, and the parameter is an image.\nExample: charterImg = {'Himeko': 'img.png'} or {'Himeko': 'img.png', 'Seele': 'img2.jpg', ...}"
            )
    ...
    if characterName:
        if isinstance(characterName, str):
            ...
            raise TypeError(
                "The name parameter must be a string, to pass multiple names, list them separated by commas.\nExample: name = 'Himeko' or name = 'Himeko, Seele',.."
            )
...
class MiHoMoCard:
    def __init__(
        ...
        try:
            self.characterImgs, self.characterName, self.characterBackground = process_input(
                characterImgs, characterName, characterBackground
            )
    ...
    async def creat(self, uid):
        ...
        for key in data.characters:
            ...
            if self.characterName:
                if (
                    not key.name.lower() in self.characterName
                    and not str(key.id) in self.characterName
                ):
                    continue
            ...
            if self.template == 1:
                task.append(
                    one.Create(
                        ...
                        self.hide,
                        int(uid),
                        remove_html_tags(data.player.nickname),
                        self.background,
                    ).start()
                )
                ...
                if not position is None:
                    for z in character.relics:
                        if z.id[-1:] == str(position):
                            result["relic"] = [
                                await relics.creat(
                                    z, character.id, position, name_charter=character.name
                                )
                            ]
                else:
                    relic_tasks = [
                        relics.creat(key, character.id, key.id[-1:], name_charter=character.name)
                        for key in character.relics
                    ]
...
class StarRaillScore:
    def __init__(self, chart_id, relic) -> None:
        ...
        if not relic.position:
            raise TypeError(
                "Pass an object of the RelicData class, you can get it using the method: get_relic()"
            )

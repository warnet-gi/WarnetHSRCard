...
class Create:
    ...
    async def create_open_icon(self, icon_key, background, position):
        icon = await pill.get_local_asset(icon_key, size=(123, 124))
        bg = _of.open.copy()
    ...
    async def create_charters(self):
        ...
        if self.img:
            ...
            character_image = await pill.get_local_asset(self.character.preview)
            bg = bg_target.copy()
    ...
    async def create_charters_image(self, frame, bg):
        path = await pill.get_local_asset(self.character.path.icon, size=(40, 36))
        element = await pill.get_local_asset(self.character.element.icon, thumbnail_size=(46, 46))
    ...
    async def create_light_cone(self):
        ...
        image = await pill.get_local_asset(self.character.light_cone.portrait, size=(203, 276))
    ...
    async def create_light_cone_image(self, frame, bg):
        path = await pill.get_local_asset(self.character.light_cone.path.icon, size=(38, 39))
    ...
    async def create_stats(self):
        ...
        for i, attribute in enumerate(combined_attributes.values()):
            icon = await pill.get_local_asset(attribute.icon, size=(32, 34))
    ...
    async def create_skill_icon(self, bg, skill, position):
        ...
        icon, font = await asyncio.gather(
            pill.get_local_asset(skill.icon, size=(35, 30)), pill.get_font(14)
        )
    ...
    async def create_dop_stats_icon(self, bg_full, key, res, position, y):
        ...
        icon_stats = await pill.get_local_asset(key.icon, size=(35, 35))
        bg.alpha_composite(icon_stats, (3, 5))

        for i, icon in enumerate(data_dop):
            icons = await pill.get_local_asset(icon, size=(29, 29))
    ...
    async def create_set_image(self, set_data, position, font, fontC):
        ...
        d = ImageDraw.Draw(frame)
        icon = await pill.get_local_asset(set_data["icon"], size=(56, 56))
        if set_data["properties"] is not None:
            icon_stats = await pill.get_local_asset(set_data["properties"]["icon"], size=(23, 24))
    ...
    async def create_relics(self, relics):
        ...
        icon_rel = await pill.get_local_asset(relics.icon, size=(59, 53))
        icon = await pill.get_local_asset(relics.main_affix.icon, size=(25, 26))
    ...
    async def create_sub_affix_image(self, sub_affix, bg, position):
        icon = await pill.get_local_asset(sub_affix.icon, size=(25, 26))

...
class Creat:
    ...
    async def creat_lk(self):
        ...
        x = int(
            font_18.getlength(f"{self.lang.lvl}:{self.character.light_cone.level}/{max_level}") / 2
        )
        d.text(
            (229 - x, 63),
        ...
        names = await pill.create_image_with_text(
            self.character.light_cone.name, 18, max_width=297, color=(255, 255, 255, 255)
        )
    ...
    async def creat_charters(self):
        if self.img:
            ...
            bg = await pill.creat_bg_teample_two(
                self.character.portrait, bg, maska=_of.maska_background.convert("L"), teample=4
            )
    ...
    async def create_sets(self):
        ...
        for i, key in enumerate(rel_set):
            ...
            d.text(
                (int(315 - size / 2), position[i]),
                sets["name"],
                font=sets_name_font,
                fill=(109, 255, 143, 255),
            )
        ...
        for attribute in combined_attributes:
            ...
            if attribute in dop:
                ...
                name_text = await pill.create_image_text(
                    f"{stat.name}:", 25, max_width=199, max_height=34, color=(255, 255, 255, 255)
                )
                bg_main_frame.alpha_composite(
                    name_text, (position_icon_main[im][0] + 40, position_text_main[im][1])
                )

                x = position_text_main[im][0] - int(font.getlength(str(value)))
                d.text(
                    (x, position_text_main[im][1]), str(value), font=font, fill=(255, 255, 255, 255)
                )

                x = position_text_main[im][0] - int(font_dop.getlength(dop[attribute]["dop"]))
                d.text(
                    (x, position_text_main[im][1] + 26),
                    dop[attribute]["dop"],
                    font=font_dop,
                    fill=(229, 195, 144, 255),
                )
                ...
                name_text = await pill.create_image_text(
                    f"{stat.name}:", 25, max_width=533, max_height=34, color=(255, 255, 255, 255)
                )
    ...
    async def creat_seeleland(self):
        ...
        draw.text(
            (127, 38), data["percrank"].replace("top ", ""), font=font, fill=(255, 255, 255, 255)
        )

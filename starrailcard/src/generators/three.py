...
class Creat:
    ...
    def __init__(
        self, characters, lang, img, hide, uid, seeleland, characterBackgroundimg, backgroundBlur
    ) -> None:
    ...
    async def creat_lk(self):
        ...
        image = await pill.get_local_asset(self.character.light_cone.portrait, size=(151, 213))
        ...
        d.text(
            (5, -1),
            await ups(self.character.light_cone.rank),
            font=font_17,
            fill=(247, 212, 168, 255),
        )
        d.text(
            (28, 29),
        ...
        names = await pill.create_image_with_text(
            self.character.light_cone.name, 20, max_width=214, color=(255, 255, 255, 255)
        )
    ...
    async def creat_charters(self):
        ...
        if self.img:
            bg = await pill.creat_user_image_tree(
                self.img, self.characterBackgroundimg, self.backgroundBlur
            )
        else:
        ...
        for key in self.character.skill_trees:
            if key.max_level == 1:
                if key.anchor in ["Point05", "Point06", "Point07", "Point08"]:
                    icon = await pill.get_local_asset(key.icon, size=(38, 38))
                else:
                    icon = await pill.get_local_asset(key.icon, size=(30, 30))
                ...
                icon = await pill.get_local_asset(key.icon, size=(43, 43))
                icon = await pill.recolor_image(icon, (247, 212, 168))
    ...
    async def creat_stats(self):
        ...
        for attribute in combined_attributes:
            ...
            icon = await pill.get_local_asset(stat.icon, size=(58, 58))
            ...
            if attribute in dop:
                ...
                name_text = await pill.create_image_text(
                    stat.name, 30, max_width=249, max_height=34, color=(255, 255, 255, 255)
                )
                ...
                x = 418 - int(font.getlength(str(value)))
                dd.text(
                    (x, position_text_dop_stats), str(value), font=font, fill=(255, 255, 255, 255)
                )
    ...
    async def create_sub_affix_image(self, sub_affix, bg, position):
        icon = await pill.get_local_asset(sub_affix.icon, size=(26, 26))
    ...
    async def creat_relics(self, relics):
        ...
        icon_rel = await pill.get_local_asset(relics.icon, size=(104, 108))
        icon = await pill.get_local_asset(relics.main_affix.icon, size=(36, 36))
        ...
        position = ((193, 20), (299, 20), (193, 62), (299, 62))

        for i, k in enumerate(relics.sub_affix):
            icon = await pill.get_local_asset(k.icon, size=(36, 36))
            bg.alpha_composite(icon, position[i])
            d.text(
                (position[i][0] + 40, position[i][1]),
                str(k.display),
                font=font,
                fill=(255, 255, 255, 255),
            )
    ...
    async def create_sets(self):
        ...
        for i, key in enumerate(rel_set):
            ...
            sets = rel_set[key]
            icon = await pill.get_local_asset(sets["icon"], size=(42, 42))
    ...
    async def creat_info(self):
        ...
        icon_element = await pill.get_local_asset(
            self.character.element.icon, thumbnail_size=(40, 39)
        )
        path = await pill.get_local_asset(self.character.path.icon, thumbnail_size=(41, 41))
        ...
        draw.text(
            (int(193 - size / 2), 89), path_name, font=sets_name_font, fill=(255, 255, 255, 255)
        )

...
_DEFAULT_SCORE = {'count': 0, 'rolls': {}, 'rank': {'name': 'N/A', 'color': (255, 255, 255, 255)}}
...
class Creat:
    ...
    async def creat_bacground(self):
        ...
        if self.img:
            ...
            line, self.element_color = await pill.recolor_image(
                _of.background_line, self.element_color[:3], light=True
            )
        else:
            bg = _of.background_default.copy()
            user_image = await pill.get_centr_honkai(
                (850, 782), await pill.get_local_asset(self.character.portrait)
            )
        ...
        if self.img:
            self.background = ImageChops.soft_light(
                self.background, _of.background_overlay.convert("RGBA")
            )
    ...
    async def creat_light_cone(self):
        ...
        image = await pill.get_local_asset(self.character.light_cone.portrait, size=(118, 169))
        ...
        d.text(
            (28, 4),
            self.character.light_cone.attributes[0].display,
            font=font_18,
            fill=(255, 255, 255, 255),
        )
        d.text(
            (28, 30),
            self.character.light_cone.attributes[1].display,
            font=font_18,
            fill=(255, 255, 255, 255),
        )
        d.text(
            (28, 56),
            self.character.light_cone.attributes[2].display,
            font=font_18,
            fill=(255, 255, 255, 255),
        )
        ...
        d.text(
            (165, 69),
            f"{self.lang.lvl}: {self.character.light_cone.level}/{max_level}",
            font=font_18,
            fill=(255, 255, 255, 255),
        )

        names = await pill.create_image_with_text(
            self.character.light_cone.name, 18, max_width=180, color=(255, 255, 255, 255)
        )
        line = Image.new(
            "RGBA",
            (1, 48),
            color_lc_line.get(str(self.character.light_cone.rarity), (150, 202, 255, 255)),
        )
    ...
    async def creat_name(self):
        ...
        names = await pill.create_image_with_text(
            self.character.name, 25, max_width=262, color=(255, 255, 255, 255)
        )
        if names.size[1] <= 53:
    ...
    async def creat_stats(self):
        ...
        for i, attribute in enumerate(combined_attributes):
            ...
            icon = await pill.get_local_asset(stat.icon, size=(45, 45))
            background = Image.new("RGBA", (273, 58), (0, 0, 0, 0))
            d = ImageDraw.Draw(background)
            value = "{:.1f}%".format(stat.value * 100) if stat.percent else round(stat.value)
            name_text = await pill.create_image_text(
                stat.name, 15, max_width=140, max_height=45, color=(255, 255, 255, 255)
            )
    ...
    async def creat_relic(self, relic):
        ...
        image = await pill.get_local_asset(relic.icon, size=(113, 113))
        ...
        main_stat_icon = await pill.get_local_asset(relic.main_affix.icon, size=(48, 48))
        ...
        for k in relic.sub_affix:
            icon = await pill.get_local_asset(k.icon, size=(29, 29))
            ...
            if k.type in self.score_info["bad"]:
                ...
                d.text(
                    ...
                    str(k.display),
                    font=font_18,
                    fill=color_scoreR.get(scoreR, (255, 255, 255, 255)),
                )
            d.text(
                (232, y_roll),
                f"+{scoreR}",
    ...
    async def creat_score_total(self):
        ...
        if self.element_color[:3] == (255, 255, 255):
            ...
            d.text(
                (x, -1), f"{round(result_percentage,1)}%", font=font_18, fill=(255, 255, 255, 255)
            )
        ...
        d.text(
            (320, -2),
            str(self.score_info["total_score"]['count']),
            font=font_21,
            fill=self.score_info["total_score"]["rank"]["color"],
        )

        d.text((17, -2), "Summary Rank:", font=font_21, fill=(255, 255, 255, 255))
        d.text(
            (189, -2),
            str(self.score_info["total_score"]["rank"]["name"]),
            font=font_21,
            fill=self.score_info["total_score"]["rank"]["color"],
        )
        ...
        d.text(
            ...
            str(self.totall_eff),
            font=font_21,
            fill=color_scoreR.get(self.totall_eff, (255, 255, 255, 255)),
        )
    ...
    async def creat_path(self):
        ...
        for key in self.character.skill_trees:
            if key.anchor in ['Point01', 'Point02', 'Point03', 'Point04', 'Point05']:
                icon = await pill.get_local_asset(key.icon, size=(47, 47))
                ...
                d.text(
                    (x, 1), f"{key.level}/{key.max_level}", font=font_15, fill=(255, 255, 255, 255)
                )
                ...
                icon = await pill.get_local_asset(key.icon, size=(35, 35))
                icon = await pill.recolor_image(icon, (0, 0, 0))
                if key.level == key.max_level:
                    self.background_path.alpha_composite(icon, path[key.anchor]["icon"])
                else:
    ...
    async def creat_constant(self):
        ...
        for key in self.character.rank_icons[:rank]:
            ...
            icon = await pill.get_local_asset(key, size=(53, 53))
    ...
    async def creat_seeleland(self):
        ...
        if self.seeleland:
            ...
            draw.text(
                (127, 42), data["rank"].replace("top ", ""), font=font, fill=(255, 255, 255, 255)
            )

    async def start(self):
        if self.img:
            self.element_color = await pill.get_background_colors(
                self.img, 15, common=True, radius=5, quality=800
            )

...
_PATH_FILE_NAME = ["max", "relic_id", "rolls", "score"]
...
class Calculator:
    ...
    async def get_relic_score(self, chara_id, relic_json):
        ...
        main_weight = self.score[chara_id]["main"][
            self.relic_id.get(relic_json.id, relic_json.id)[-1]
        ][relic_json.main_affix.type]
        main_affix_score = (relic_json.level + 1) / 16 * main_weight
        result_json["main_formula"] = (
            f'{round((relic_json.level + 1) / 16 * 100, 1)}×{main_weight}={main_affix_score * 100}'
        )
        ...
        for sub_affix_json in relic_json.sub_affix:
            ...
            sub_affix_formulas.append(
                f'{round(sub_affix_json.value / self.max[sub_affix_json.type] * 100, 1)}×{round(self.score[chara_id]["weight"][sub_affix_json.type], 1)}'
            )
        sub_affix_score = sum(
            (sub_affix_json.value / self.max[sub_affix_json.type])
            * self.score[chara_id]["weight"][sub_affix_json.type]
            for sub_affix_json in relic_json.sub_affix
        )
